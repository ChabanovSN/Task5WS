...
router.get(BASE_URL, async (ctx) => {
  ...
  router.get(`${BASE_URL}/:id`, async (ctx) => {
    try {
        const movie = await queries.getSingleMovieById(ctx.params.id);
        if (movie.length) {
          ctx.body = {
            status: 'success',
            data: movie
  ...
  router.get(`${BASE_URL}/single/:name`, async (ctx) => {
    try {
     
        const movie = await queries.getSingleMovieByName(ctx.params.name);
        if (movie.length) {
          ctx.body = {
            status: 'success',
            data: movie
